...
int main()
{
	...
	listnode.InsertNode(head, pos, val);
	listnode.PrintList(head);
	std::cout<<"当前长度: "<<listnode.GetLen()<<std::endl;
	pos = 6;
	ElementData data = listnode.GetNode(head, pos);
	std::cout<<"获得第"<<pos<<"个位置值: "<<data<<std::endl;
	pos = 4;
	val = 6;
	listnode.UpdateNode(head, pos, val);
	std::cout<<"更新第"<<pos<<"个位置值为: "<<val<<std::endl;
	listnode.PrintList(head);
	std::cout<<"当前长度: "<<listnode.GetLen()<<std::endl;
	pos = 4;
	listnode.DeleteNode(head, pos);
	std::cout<<"删除第"<<pos<<"个位置值"<<std::endl;
	listnode.PrintList(head);
	std::cout<<"当前长度: "<<listnode.GetLen()<<std::endl;
	val = 12;
	pos = listnode.SearchNode(head, val);
